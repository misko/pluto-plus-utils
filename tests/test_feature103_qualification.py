...
def test_dry_run_is_rc12_receipt_gated_and_non_mutating(tmp_path: Path) -> None:
    request = _request(tmp_path)
    result = run_feature103_qualification(request, execute=False)

    assert result["mode"] == "dry_run"
    assert result["plan"]["ram_boot"]["dfu_sha256"] == FEATURE_103_RC12_DFU_SHA256
...
def test_execute_requires_confirmation_and_writes_gate_evidence(tmp_path: Path) -> None:
    ...
    assert len(calls) == 2
    assert calls[1][2]["candidate_fit_sha256"] == FEATURE_103_RC12_FIT_SHA256
