...
def test_reboot_local_defaults_to_read_only_plan(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert document["plan"]["expected_return_firmware"] is None


def test_reboot_local_binds_hardware_qualified_return_profile(
...
def test_reboot_local_rejects_unqualified_return_profile() -> None:
    result = runner.invoke(
        ...
        [
            ...
            "/private/radio.known_hosts",
            "--expect-return-profile",
            "ddr-capacity-test-rc1-ram",
        ],
    )

    assert result.exit_code == 2
    error = json.loads(result.stderr)["error"]
    assert error["code"] == "local_reboot_return_profile_invalid"
