...
def test_issue_72_v3_final_trusted_bytes_require_distinct_persistent_promotion() -> None:
    ...
    assert policy.fit_body_size == 12_825_571
    assert policy.hardware_qualified is False
    assert profile.persistent_allowed is False
    assert profile.metadata_abi == 3
    assert profile.tandem_agc is True
...
def test_rx_only_persistent_canary_is_local_only_and_has_exact_1r1t_transition() -> None:
    ...
    assert promotion.persistent_allowed is profile.persistent_allowed is True
    assert promotion.metadata_abi == profile.metadata_abi == 3
    assert promotion.tandem_agc is profile.tandem_agc is False
