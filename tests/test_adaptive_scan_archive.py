...
def test_archive_seals_exact_complete_iq(tmp_path):
    setup = build_adaptive_scan_setup(
        session=7, generation=8, seed=9, source_rate_hz=10_000_000,
        analog_bandwidth_hz=10_000_000, duration_ms=300_000, dwell_ms=120,
        frequencies_hz=(1_190_312_500,), baseline_weights=(1,), analysis_digest=b"x" * 32,
    )
    record = ScanVisit(
        session=7, generation=8, visit=0, target=0, profile=1,
        result=VisitResult.COMPLETE, frequency_hz=1_190_312_500,
        source_rate_hz=10_000_000, analog_bandwidth_hz=10_000_000,
        selection_counter=90, transition_before=100, transition_after=180,
        valid_start=200, valid_end=204, missing_samples_before=0,
        eligible_mask=1, effective_weight=1, profile_crc32=1, iq_bytes=16,
    )
    ...
    terminal = ScanTerminal(
        session=7, generation=8, planned=1, delivered=1, skipped=0, invalid=0,
        cancelled=0, final_counter=204, restore_before=205, restore_after=206,
        iq_bytes=16, state=TerminalState.COMPLETED, reason=0, error=0,
    )
