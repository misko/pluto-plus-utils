...
(equal probability), and manual/slow_attack gain (equal probability). Domain-
separated SHA-256 rejection sampling makes retries reproducible. Manual gain
remains 40 dB. The selected lower/upper edge policy is unchanged.
...
The client refuses old firmware rather than silently falling back. Automatic
gain is programmed on both receivers and read back; manual gain values are
