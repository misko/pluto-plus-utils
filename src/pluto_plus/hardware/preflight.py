...
V7_FIRMWARE_VERSION = "v0.40-plutoplus-spf-tandem-agc-v7"


@dataclass(frozen=True, slots=True)
class MetadataRuntimeProfile:
    """One reviewed firmware/host-runtime pairing."""

    metadata_abi: int
    firmware_version: str | None
    source_ref: str
    source_commit: str
    metadata_buffer_parameters: tuple[str, ...]
...
METADATA_RUNTIME_PROFILES = {
    (1, None): MetadataRuntimeProfile(
        metadata_abi=1,
        firmware_version=None,
        source_ref="spf-frame-metadata-source/v0.25-final-v3",
        source_commit="c26258bfa33098c2b215e19cf85d448e89499b1a",
        metadata_buffer_parameters=METADATA_BUFFER_PARAMETERS[1],
    ),
    (2, V7_FIRMWARE_VERSION): MetadataRuntimeProfile(
        metadata_abi=2,
        ...
        metadata_buffer_parameters=METADATA_BUFFER_PARAMETERS[2],
    ),
}
...
def verify_metadata_runtime(
    ...
    *,
    ...
    profile = _metadata_runtime_profile(expected_abi, expected_firmware_version)
    ...
    if parameters != profile.metadata_buffer_parameters:
        raise RuntimeError(
            f"MetadataBuffer constructor is {parameters}, expected "
            f"{profile.metadata_buffer_parameters}"
        )
...
def _metadata_runtime_profile(
    expected_abi: int,
    expected_firmware_version: str | None,
) -> MetadataRuntimeProfile:
    if expected_abi not in {1, 2}:
        raise ValueError("expected metadata ABI must be 1 or 2")
    if expected_abi == 2 and expected_firmware_version is None:
        raise ValueError("expected firmware version is required for metadata ABI 2")
    profile = METADATA_RUNTIME_PROFILES.get((expected_abi, expected_firmware_version))
    if profile is None:
        raise ValueError(
            "unsupported metadata runtime profile: "
            f"firmware={expected_firmware_version!r}, ABI={expected_abi}"
        )
    return profile
...
def _validate_metadata_runtime_receipt(
    ...
    *,
    prefix: Path,
    receipt_path: Path,
    profile: MetadataRuntimeProfile,
) -> MetadataRuntimeVerification:
    if not isinstance(document, dict) or document.get("schema_version") != 1:
        raise RuntimeError("metadata runtime receipt has an unsupported schema")
    if document.get("metadata_abi") != profile.metadata_abi:
        raise RuntimeError("metadata runtime receipt ABI does not match the radio")
    if document.get("firmware_version") != profile.firmware_version:
        raise RuntimeError("metadata runtime receipt has the wrong firmware profile")
    source_ref = str(document.get("source_ref") or "")
    if source_ref != profile.source_ref:
        raise RuntimeError("metadata runtime receipt has the wrong source ref")
