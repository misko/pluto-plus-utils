...
class IioMetadataCaptureSession:
    ...
    def _read_block(self) -> SampleBlockV2:
        ...
        for startup_discard in range(MAX_STARTUP_FRAME_DISCARDS + 1):
            try:
                ...
                if error.errno != errno.EAGAIN or startup_discard == MAX_STARTUP_FRAME_DISCARDS:
                    raise
                # pyadi's libiio-v0 compatibility path clears ``_rxbuf`` on
                # every refill exception.  V9 deliberately returns EAGAIN for
                # startup frames which predate the new metadata generation, so
                # restore the still-owned MetadataBuffer before retrying.  If
                # left unset, pyadi silently creates an ordinary Buffer and the
                # IQ/header pairing contract is lost.
                self._sdr._rxbuf = self._buffer
        ...
        if self._metadata_abi == 1:
            ...
            parsed = RadioMetadataV4.unpack(raw_metadata)
            self._validate_tandem_header(parsed)
    ...
    def _validate_header(self, metadata: Any) -> None:
        ...
        if metadata.flags & required_flags != required_flags:
            raise RuntimeError(
                "IIO metadata lacks a valid sample sequence or FPGA sample counter"
            )

    def _validate_tandem_header(self, metadata: RadioMetadataV4) -> None:
        expected_state = (
            TandemState.ARMED_HOLD
            if self._tandem_request.mode is TandemMode.HOLD
            else TandemState.ARMED_AUTO
        )
        if metadata.tandem_state is not expected_state:
            raise RuntimeError(
                "metadata tandem state does not match the requested ownership mode"
            )
        if metadata.tandem_fault_flags:
            raise RuntimeError(
                f"metadata tandem ownership reports fault flags "
                f"0x{metadata.tandem_fault_flags:x}"
            )
        if self._tandem_request.mode is TandemMode.HOLD and (
            metadata.tandem_transition_count or metadata.gain_events
        ):
            raise RuntimeError("metadata HOLD capture unexpectedly changed receiver gain")
        if self._tandem_request.mode is TandemMode.HOLD:
            expected_gain = self._tandem_request.initial_gain_db
            observed_gains = (
                metadata.initial_gain_db,
                metadata.base.rx1_gain_db_start,
                metadata.base.rx2_gain_db_start,
                metadata.base.rx1_gain_db_end,
                metadata.base.rx2_gain_db_end,
            )
            if any(gain != expected_gain for gain in observed_gains):
                raise RuntimeError(
                    "metadata HOLD gain does not match the requested receiver gain"
                )
